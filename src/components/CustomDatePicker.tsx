...
export default function CustomDatePicker({ selectedDate, onChange, busySlots: propSlots }: DatePickerProps) {
    ...
    const checkIsBusy = (horaStr: string) => {
        ...
        return finalSlots.some(slot => {
            ...
            const [ch, cm]           = timePart.replace(/\.\d+$/, '').split(':').map(Number);
    ...
    return (
        <div className="p-4 bg-zinc-950 rounded-[2rem] border border-zinc-800 shadow-2xl">
            ...
            {/* 🔍 DEBUG VISUAL — borra esto después */}
            <p className="text-[9px] text-yellow-400 text-center mb-1">
                Día: {currentDay.getFullYear()}-{currentDay.getMonth()+1}-{currentDay.getDate()}
            </p>
            <p className="text-[9px] text-yellow-400 text-center mb-2 break-all px-2">
                Slots: {finalSlots.map(s => {
                    const raw = typeof s === 'object' ? s.dateList : s;
